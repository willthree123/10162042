...
void Q2()
{
    ...
    //Set cout result in 6 decimal places and use "Default format"
    cout<< defaultfloat;
    cout<< setprecision(6);

	//Ask user to input three numbers a, b, c
    cout << "Enter a b c: ";
    cin >> a >> b >> c; //Input a b c

    //Calcuate if there is/are any solution
    if( pow(b,2)-(4*a*c)>0 ){ //If there are two solution
...
{
    //Declare the necessary variables
    int Amount; //To store user input
    int day = 0;
    
	//Ask user to input starting money
    cout <<"Starting money: $";
    cin >> Amount; //Input x
    ...
    while (Amount >0){//Run until using up all the money
        ...
        if(day!=0){//cout "newline" except the first line
            cout << "\n";
        }
...
{
	//Declare the necessary variables
    int size, firstNumber, tmpNum, tmpSize; //size, firstNumber are used to save user inputs. tmpNum tmpSize are used to temporary save number and size.
    char symbol = '*'; //store the symbol that needed to print.

	//Ask user to input size and first number
    cout <<"Size: ";
    cin >> size; //Input size
    cout << "First number: ";
    ...
    //Display output
    tmpSize = size; //Store size to tmpSize
    for(int j=0; j < size; j++){ 
        if(j!=0){ //cout "newline" except the first line
            cout << "\n";
        }
        tmpNum=firstNumber;
        for (int k=0; k<(size-tmpSize); k++){ //cout space(in proper format)
            cout << "  "; 
        }
        ...
        tmpSize--; //The end of one line(size)
    }
    //The end of question 
}
